...
<body> <!-- Start of the visible area of the website -->
    ...
    <div class="content"> <!-- Main container for the actual content -->
        ...
        <div class="results"> <!-- Area where search results are displayed -->
            ...
            if (!empty(trim($_POST['search'] ?? ''))) {
                    ...
                    if ($direct) { // If a direct hit was found...
                        ...
                        if (!empty($offName)) {
                            ...
                            while ($row = $stmtFuzzy->fetch(PDO::FETCH_ASSOC)) { // Go through all products in the database...
                                ...
                                // Weighted total score (70% word score, 30% Levenshtein)
                ...
                // 3. Display in Cyber Terminal
                ?> <!-- End of PHP logic part -->
                <div class="terminal-container"> <!-- Outer container for the terminal design -->
                    ...
                    <div class="terminal-body"> <!-- Main content of the terminal -->
                        <div class="terminal-content" id="terminal-content">
                            ...
                            <!-- Condition: Show product if (hit in DB and score >= 25%) OR online data available -->
                            <?php if (($finalMatch && $score >= 25) || $offData): ?>
                                <div class="terminal-line"> <!-- Simulates a command line -->
                                ...
                                <div class="output"> <!-- Container for the result output -->
                                    ...
                                    <div class="cyber-product-name"> <!-- Line for the product name -->
                                        <!-- Show DB name, if not available API name, otherwise 'UNKNOWN' -->
                                        <?php echo htmlspecialchars($finalMatch['name'] ?? $offName ?? 'UNKNOWN'); ?>
                                    </div> <!-- End name -->
                                    <?php if (isset($finalMatch['preis'])): ?>
                                        <!-- If a price was found in the DB... -->
                                        <div class="cyber-price-tag"> <!-- Price display -->
                                            <!-- Calculate gross (price * 1.19) and format to 2 decimal places -->
                                            <?php echo number_format($finalMatch['preis'] * 1.19, 2, ',', '.'); ?> €
                                        </div> <!-- End price -->
                                    <?php else: ?> <!-- If no price was found (e.g., online hit only) -->
                                        <div class="cyber-price-tag glitch">NO_PRICE</div> <!-- Show error with glitch effect -->
                                    <?php endif; ?>

                                    <?php if ($score > 0): ?> <!-- If there is a fuzzy match result... -->
                                        <div class="info-row"> <!-- Info line -->
                                            <span class="info">► Match Score: <?php echo round($score, 1); ?>%</span>
                                            <!-- Show the calculated match -->
                                        </div> <!-- End info line -->
                                    <?php endif; ?>
